...
export default function Edit_Interview_Posts({user, isAdmin, isLogged}) {
  ...
  const validityCheck = (salary, work_hours, title, job_description, post_picture) =>{
      ...
      if(!validateWorkHours(work_hours)){
  ...
  const onSubmit = async (data) =>{
    ...
    if(checkEmpty){
      const checkValid = validityCheck(salary, work_hours, title, job_description, post_picture);
      if (checkValid){
          post_picture = picToSend;
          try {
            const res = await axios.patch('/user/edit-post', {
                postId, salary, title, job_description, post_picture, work_hours, skills, location, experience, qualification, careerLevel, expiryDate, expiryTime
            })
            setError("")
        ...
        <div className={classes.paper}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                ...
                <h4> Post Picture </h4>
                <input required type="file" {...register("picture") } name = "picture" id = "picture" className= "choose__file__button" onChange={changeAvatar}/>
              </Grid>
