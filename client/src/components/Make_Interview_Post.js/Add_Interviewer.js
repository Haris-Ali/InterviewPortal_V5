...
export default function Add_Interviewer({user, isAdmin, isLogged}) {
 ...
 const yearsArray = [
  ...
  const validityCheck = (
      ...
      //Company
      else if (!validateCompanyName(company_name)){
  ...
  const onSubmit = async (data) =>{
    ...
    if (checkEmpty){
      ...
      if (checkValid){
        ...
        try {
          const res = await axios.patch('/user/add-interviewer-by-userId', {
            userId : user._id,
            age,
            city,
            phone_number,
