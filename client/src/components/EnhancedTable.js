...
export default function EnhancedTable({user, testsReceived}) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = tests.map((test) => test._id);
      setSelected(newSelecteds);
   
      return;
    }
    setSelected([]);
    console.log("selected outside of all =", selected)
  };
  ...
  useEffect(() => {
    console.log("selected abhi = ", selected)
    axios
              ...
              {stableSort(tests, getComparator(order, orderBy))
                ...
                .map((test, index) => {
                  ...
                  return (
                    ...
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, test._id)} 
                      role="checkbox"
